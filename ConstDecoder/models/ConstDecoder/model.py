...
class TagDecoder(nn.Module):
    def __init__(self, args):
        ...
        self.decoder_proj = nn.Sequential(
            nn.Linear(args.tag_hidden_size, args.vocab_size), nn.Softmax(dim=-1)
        )
    ...
    def forward(self, inputs):
        src_token, src_mask, src_pos, target = (
            inputs["src_token"].to(self.args.device),
        ...
        tag_weight = torch.ones(
            self.args.tag_size, dtype=torch.float, device=self.args.device
        )
        tag_weight[-1] = self.args.change_weight

        tag_loss = F.cross_entropy(
            edit_tags.view(-1, self.args.tag_size),
            tag_labels.view(-1),
            weight=tag_weight,
        )
        ...
        if target.size(-1) == 1:
            ...
            raise ValueError(
                "Check the preprocess, must be the form of '[CLS] ... [SEP]' "
            )
        else:
            batch_memory = []
            batch_memory_mask = []
                ...
                for token_id in range(src_len):
                    if target[batch_id][token_id][0] != 2:  # if not change
                        ...
                        batch_changes.append(
                            src_encode[batch_id][token_id].unsqueeze(0)
                        )

                        batch_gen_input_ids.append(
                            target[batch_id][token_id][1:-1].unsqueeze(0)
                        )
                        batch_gen_mask.append(
                            target_mask[batch_id][token_id][1:-1].unsqueeze(0)
                        )
                        batch_gen_labels.append(
                            target[batch_id][token_id][2:].unsqueeze(0)
                        )
            ...
            batch_changes = batch_changes.unsqueeze(1).repeat(
                1, gen_input_embeds.size(1), 1
            )
            ...
            tgt_mask = nn.Transformer.generate_square_subsequent_mask(
                fused_gen_input.size(1)
            )
            decoder_outputs = self.edit_decoder(
                tgt=fused_gen_input,
                memory=batch_memory,
            ...
            gen_loss = F.nll_loss(
                gen_probs.log().view(-1, self.args.vocab_size),
                batch_gen_labels.view(-1),
    ...
    def generate(self, input_str):
        ...
        if not batch_id_token_id2count_id:  # empty dict
            ...
            # start symbol 101
            batch_gen_input_ids = (
                torch.ones(batch_changes.size(0), 1)
                .fill_(self.bert_tokenizer.cls_token_id)
                .type(torch.long)
                .to(self.args.device)
            )
            for i in range(self.args.max_add_len - 1):
                gen_input_embeds = self.bert_encoder.embeddings(batch_gen_input_ids)
                batch_changes_new = batch_changes.repeat(
                    1, gen_input_embeds.size(1), 1
                )  # same length
                ...
                batch_gen_input_ids = torch.cat(
                    [batch_gen_input_ids, next_word[:, -1].unsqueeze(1)], dim=1
                )

            batch_gen_input_ids = batch_gen_input_ids.tolist()
            for k, v in batch_id_token_id2count_id.items():
                end_pos = (
                    batch_gen_input_ids[v].index(self.bert_tokenizer.sep_token_id)
                    if self.bert_tokenizer.sep_token_id in batch_gen_input_ids[v]
                    else -1
                )
